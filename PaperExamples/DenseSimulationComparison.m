...
for ii = 2:2
   ...
   a = A(ii); % absolute value of parameters
    ...
    D = [12 12 12 12]; %pth of lambda log_0.5
    ...
    lambda3 = 0.5.^linspace(-5,D(3),L(3));
    lambda4 = 0.5.^linspace(-5,D(4),L(4));
...
parlist = pargen_unif_new(adj, p, q, a, b, 0.6);
...
[idv3_sens(:,k) idv3_spec(:,k) idv_tp idv_tn] = fitcompare(fitlist_post, parlist);
 ...
 %%%%%%%% FIT calibrated version of Lee & Hastie  %%%%%%%%%%%%%%%%%%
              ...
              filename = sprintf('Simulation/Dense_comparestudy_seeda=%.1f_b=%.1f%d_%s.mat',a,b,seed, char(model(i)));
...
ylabel('Overall TPR','FontSize',20);
              ...
              [y1,f1] = csaps(1-vec_adj1_spec_zz,vec_adj1_sens_zz(id1),0.25,1-vec_adj1_spec_zz);
              [y3,f3] = csaps(1-vec_adj3_spec_zz,vec_adj3_sens_zz(id3),0.25,1-vec_adj3_spec_zz);
              [y4,f4] = csaps(1-vec_adj4_spec_zz,vec_adj4_sens_zz(id4),0.25,1-vec_adj4_spec_zz);
              [y5,f5] = csaps(1-vec_adj5_spec_zz,vec_adj5_sens_zz(id5),0.25,1-vec_adj5_spec_zz);
