...
function parlist = pargen_unif(adjmat, p, q, a, b)
%% function to generate parameters according to graph. b is for Phi's and a is for all other parameters
%% parameters are generated from uniformat distribution between 0.9*a and 1.1*a
...
parlist(1).lambda_j = a*sign(unifrnd(-1,1,q,1)).*unifrnd(0.9,1.1,q,1);
parlist(1).lambda_jk = a*sign(unifrnd(-1,1,q*(q-1)/2,1)).*unifrnd(0.9,1.1,q*(q-1)/2,1);
