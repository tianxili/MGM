...
function [fitlist_post fitlist a0_lr a0_ls beta_lr beta_ls sigma2] = sepreg_weight_flex(z,y,tune1,tune2,method,kappa)
% Input
% [z y]: data
% p: dim of y
% q: dim of z
...
for j = 1:q
    ...
    options.penalty_factor = penalty_fac_flex('z',p,q,kappa);
...
for j = 1:p
    ...
    options.standardize = false;
    options.penalty_factor = penalty_fac_flex('y',p,q,kappa);
    if (size(xtmp,2)>size(xtmp,1))
    options.type = 'naive';
    end
     % Fit the entire solution path
